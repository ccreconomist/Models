...
FROM (
    ...
    SELECT
        ...
        CASE
            WHEN LEFT(REPLACE(SUBSTRING(CUENTA, 4, LEN(CUENTA)), '000', ''), 6) = '506370' THEN 'GCC'
            --WHEN LEFT(REPLACE(SUBSTRING(CUENTA, 4, LEN(CUENTA)), '000', ''), 6) = '506329' THEN 'G&A'
            WHEN LEFT(REPLACE(SUBSTRING(CUENTA, 4, LEN(CUENTA)), '000', ''), 6) = '481282' THEN 'GCC'
            ELSE ''
        END AS SOCIO,
        CASE
            WHEN LEFT(REPLACE(SUBSTRING(CUENTA, 4, LEN(CUENTA)), '000', ''), 6) = '506370' THEN 'PLCC'
            --WHEN LEFT(REPLACE(SUBSTRING(CUENTA, 4, LEN(CUENTA)), '000', ''), 6) = '506329' THEN 'PLCC'
            WHEN LEFT(REPLACE(SUBSTRING(CUENTA, 4, LEN(CUENTA)), '000', ''), 6) = '481282' THEN 'BK'
            ELSE ''
        END AS PRODUCTO
    FROM
        IT_CARTERA_GCC
    WHERE
        LEFT(REPLACE(SUBSTRING(CUENTA, 4, LEN(CUENTA)), '000', ''), 6) IN ('506370','481282')
