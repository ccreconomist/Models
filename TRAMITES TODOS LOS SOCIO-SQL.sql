...
FROM(
	...
	WHERE Fecha_Solicitud >= '2023-06-01 00:00:00' 
...
FROM (
	SELECT ANIO, MES, Id_Socio, SOCIO, TIENDA, COUNT(Case_Number) AS TRAMITES 
	FROM ( 
		SELECT DISTINCT YEAR(Fecha_Solicitud) AS ANIO, MONTH(Fecha_Solicitud) AS MES, Id_Socio
		, CASE	WHEN Id_Socio = 0 THEN 'C&A'
					WHEN Id_Socio = 1 THEN 'BRADESCARD'
	...
	GROUP BY ANIO, MES, Id_Socio, SOCIO, TIENDA
) AS A
...
FROM (
	SELECT ANIO, MES, Id_Socio, SOCIO, PROMOTOR, COUNT(Case_Number) AS TRAMITES 
	FROM (
		SELECT DISTINCT YEAR(Fecha_Solicitud) AS ANIO, MONTH(Fecha_Solicitud) AS MES, Id_Socio
		, CASE	WHEN Id_Socio = 0 THEN 'C&A'
					WHEN Id_Socio = 1 THEN 'BRADESCARD'
		...
		FROM V_Datos_Generales
		WHERE Fecha_Solicitud >= '2023-05-01 00:00:00' 
		AND Fecha_Solicitud < '2023-09-15 00:00:00'
			AND Id_Socio IN (0,4,5,6,7)
			--AND AnalistaCaptura != 'UsronlineProm'
			AND AnalistaCaptura NOT LIKE '%Usronline%'
	) AS A
	GROUP BY ANIO, MES, Id_Socio, SOCIO, PROMOTOR
) AS A
GROUP BY ANIO, MES, Id_Socio, SOCIO
ORDER BY ANIO, MES, Id_Socio